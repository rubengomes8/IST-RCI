...
//Retorna o número de caracteres ch de string
...
int validate_stream(int argc, char *argv1, char* streamID, char* streamNAME, char *streamIP, char* streamPORT)
{
	...
	if(argc > 1)
	{
		if(strlen(argv1) > STREAM_ID_SIZE)
		{
			...
			printf("O streamID inserido tem tamanho superior ao possível!\n");
			return 0;
		}
		else if(strlen(argv1) > 2)
		{
			...
			{
				printf("Formato do streamID: <Name:IPAddress:Port\n");
				return 0;
			}
		}
		else
	...
	for (n = 1; n < argc; n+=2)
	{
		...
		else if(!strcmp(argv[n], "-p"))
		{
			...
			*tcp_sessions = atoi(argv[n + 1]);
			if(*tcp_sessions < 1)
			{
				printf("tcpsessions deve ser pelo menos 1!\n");
				exit(-1);
			}
		...
		else if(!strcmp(argv[n], "-n"))
		{
			...
			*bestpops = atoi(argv[n + 1]);
			if(*bestpops < 1)
			{
				printf("bestpops deve ser pelo menos 1!\n");
				exit(-1);
			}
			printf("bestpops: %d\n", *bestpops);
		}
		else if(!strcmp(argv[n], "-x"))
		{
			...
			*tsecs = atoi(argv[n + 1]);
			if(*tsecs < MIN_TSECS)
			{
				printf("tsecs deve ser pelo menos 1!\n");
				exit(-1);
			}
			printf("tsecs: %d\n", *tsecs);
		}
